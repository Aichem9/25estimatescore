...
with st.expander("규칙 요약", expanded=False):
    st.markdown("""
    **엑셀 규칙(고정):**
...
for band in ["상","중","하"]:
    with st.sidebar.expander(f"{band} 영역 정답률(%)", expanded=(band=="중")):
        ...
        for level in ["A","B","C","D","E"]:
            rates[band][level] = st.slider(f"{level} ({band})", 0, 100, int(default_rates[band][level]), 5) / 100.0
...
# We'll use positional indices to avoid header name ambiguity
...
def derive_band(irow):
    # Use raw_all (unaltered) to read D/E/F parallel to df rows
...
# ---------- Expected totals & cut scores ----------
levels = ["A","B","C","D","E"]
bands = ["상","중","하"]

# Item-level expected points
rows = []
...
cuts = {
    "E/D 컷": mid(exp_totals["E"], exp_totals["D"]),
    "D/C 컷": mid(exp_totals["D"], exp_totals["C"]),
    "C/B 컷": mid(exp_totals["C"], exp_totals["B"]),
    "B/A 컷": mid(exp_totals["B"], exp_totals["A"]),
}

st.subheader("수준별 기대 총점 & 추정 분할 점수")
colA, colB = st.columns(2)
with colA:
    st.markdown("**수준별 기대 총점**")
    st.dataframe(pd.DataFrame([exp_totals]), use_container_width=True)
with colB:
    st.markdown("**분할 점수(중간값)**")
