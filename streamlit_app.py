...
def is_O_mark(val):
    if pd.isna(val):
        return False
    s = str(val).strip().lower()
    # Accept common circle / O marks
    return s in {"o", "Ｏ", "○", "◯", "o.", "o/o", "o표시"} or s == "o" or "o" == s or s == "○" or s == "◯" or s == "O".lower()
...
# 체크박스 기반 난이도 해석 (수동 매핑 우선 → 자동 매핑)
def get_col_if_valid(name):
    return name if (name and name != "(자동)" and name in df.columns) else None

col_hard = get_col_if_valid(opt_hard)
col_mid  = get_col_if_valid(opt_mid)
col_easy = get_col_if_valid(opt_easy)

cols_lower = {c.lower(): c for c in df.columns}
auto_hard = cols_lower.get("어려움")
auto_mid  = cols_lower.get("보통")
auto_easy = cols_lower.get("쉬움")

if not col_hard and auto_hard: col_hard = auto_hard
if not col_mid  and auto_mid:  col_mid  = auto_mid
if not col_easy and auto_easy: col_easy = auto_easy

if ('band' in locals() and pd.notna(pd.Series(band)).sum() > 0):
    pass
elif col_hard and col_mid and col_easy:
    ...
    # 폴백: 난이도 텍스트/수치 매핑 (col_diff가 없을 수 있으므로 안전 체크)
    try:
        col_item_fallback, col_diff_fb, _ = find_cols(df)
        if col_diff_fb and col_diff_fb in df.columns:
            diff_col = df[col_diff_fb]
            if diff_col.apply(is_number).mean() > 0.6:
                band = map_numeric_to_band(diff_col)
            else:
                band = map_text_to_band(diff_col)
        else:
            band = pd.Series([None]*len(df), index=df.index)
    except Exception:
        band = pd.Series([None]*len(df), index=df.index)
...
items = df[col_item] if (col_item and col_item in df.columns) else pd.Series(range(1, len(df)+1), name="문항")
